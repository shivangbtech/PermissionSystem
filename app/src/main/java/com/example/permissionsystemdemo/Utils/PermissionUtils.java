package com.businessfriend.utilities;
...
import com.businessfriend.R;
import com.businessfriend.listeners.PermissionErrorListener;

import java.util.ArrayList;

/**
 * Created by Shivang Goel on 2/3/16.
 */
public class PermissionUtils {
    ...
    public void needPermission(final Activity activity, final String permissionString,
                               final int requestCode, final String permissionName, PermissionErrorListener permissionErrorListener) {

        if (permissionString != null && permissionName != null && requestCode != 0) {
...
//            Logger.getInstance().log_info("No need to request permission programmatically!");
            ...
            Log.e(TAG, "Check the request code is non-zero or permissionString, permissionName should not same!");
        }
    }
    ...
    public void needPermission(final Activity activity, String[] permissionString,
                               final int requestCode, String[] permissionNames, PermissionErrorListener permissionErrorListener) {

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (permissionString.length == permissionNames.length &&
                    permissionString.length > 0 && requestCode != 0) {
...
//                Logger.getInstance().log_error("size of permission, explanation msg and request code are not same!");
                permissionErrorListener.onError("Check the request code is non-zero or size of permission, explanation msg are not same!");
                Log.e(TAG, "Check the request code is non-zero or size of permission, explanation msg are not same!");
            }
        } else {
//            Logger.getInstance().log_info("No need to request permission programmatically!");
            permissionErrorListener.BeforeMarshmallow();
...
//            Logger.getInstance().log_error("Error while showing dialog!", ex);
